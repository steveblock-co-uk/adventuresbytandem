...
function ConfigureMap() {
  LoadMapData('map.xml');
  // Must call set center to display map. Can't set map type until we've done
  // this.
  ResetMap();
}

function MoveMap(index) {
  // We pan to the specified position, at a zoom level between 6 and 8. The
  // idea is that this view is wide enough to see the whole day's route, but
  // narrow enough that it still. Feels like the map 'follows' the route. (At
  // zoom 6 you can see the width of the country to maintain persepctive).
